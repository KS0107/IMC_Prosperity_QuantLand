...
class Trader:
    ...
    def resin_orders(self, order_depth: OrderDepth, fair_value: float, width: float, position: int, position_limit: int) -> List[Order]:
        ...
        # Volatility-based scaling (calculate resin volatility)
        resin_volatility = self.calculate_volatility(self.resin_prices, window=5)  # Calculate volatility based on resin prices
        volatility_scaling = max(20, int(20 / (1 + resin_volatility)))  # Adjust scaling based on resin volatility

        # Find the best bid and ask based on the order depth
        ...
        # Optimize buy orders based on the position and liquidity
        if order_depth.sell_orders:
            ...
            if best_ask < fair_value:
                ...
                if quantity > 0:
                    order_size = min(quantity, volatility_scaling)  # Limit order size based on volatility scaling
        ...
        # Adjust buy/sell based on remaining space after clearing positions
        buy_quantity = position_limit - (position + buy_volume)
        sell_quantity = position_limit + (position - sell_volume)
